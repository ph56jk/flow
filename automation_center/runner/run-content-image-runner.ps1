# Wrapper khởi động Content Image Runner trên host Windows (Scheduled Task).
#
# Bí mật chỉ nằm trong runner\.env (ACL giới hạn cho chủ máy, đã gitignore).
# Script không in bất kỳ giá trị nào ra stdout/stderr nên log không chứa secret.

$ErrorActionPreference = 'Stop'

$RepoRoot = 'C:\HaviGroup\flow-v2'
$EnvFile  = Join-Path $RepoRoot 'automation_center\runner\.env'
$Python   = Join-Path $RepoRoot '.venv\Scripts\python.exe'
$Runner   = Join-Path $RepoRoot 'automation_center\runner\content_image_runner.py'
