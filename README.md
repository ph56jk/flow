...
- **Chỉ ảnh của chính bot — nên hôm nay nhánh này chưa chạy.** ERP chỉ cho xoá comment của chính mình, nên bot chỉ nhận việc với comment có `mine = 1`. Ảnh hiện do `flow_web/service.py` đăng bằng `ERP_API_KEY`/`ERP_API_SECRET`, tức dưới danh tính người thật, nên `mine = 0` và **"👎 là xoá" không gỡ được tấm nào trong thực tế** — bot đọc được phiếu nhưng không với tới. Hàm `AgentBotClient.publish_image` là mảnh còn thiếu: nó đăng ảnh dưới danh tính bot, nhưng **chưa được nối vào luồng đăng ảnh nào**. Nối nó vào rồi nhánh mới sống. Trong lúc chờ, mỗi lượt `janitor_pass` gặp ảnh đã có phán quyết mà không phải bot đăng thì ghi một dòng `WARNING` kể tên thẻ, số ảnh mắc lại và nhắc đúng `publish_image` — một nhánh nằm im mà im lặng thì đọc y hệt một nhánh không có việc. `tests/test_agent_bot.py::DeleteBranchIsDormantTests` khoá đúng trạng thái đó.
